...
<div class="starts-testimonials-updates-form">
    <div class="updates-form-form-left">
        ...
        <img src="<?php echo P_SUBSCRIBE_URL ?>includes/images/wcupdate_email.svg" style="width: 230px;margin: 60px 0px 20px 0px;" />
...
<style>
    ...
    @font-face {
        font-family: 'Lato';
        src: url('<?php echo P_SUBSCRIBE_URL."includes/fonts/Lato-Regular.woff";?>');
    }

    #wpwrap{
        background: url('<?php echo P_SUBSCRIBE_URL;?>includes/images/update-bg.jpg');
    ...
    jQuery(document).ready(function($) {
        jQuery(document).on("click", ".updates-form button, a.form-cancel-btn", function () {
            ...
            $.ajax({
                url: ajaxurl,
                data: {
                    action: "subscribe_forms_update_status",
                    status: updateStatus,
                    nonce: '<?php echo wp_create_nonce("subscribe_forms_update_status") ?>',
                    email: jQuery("#folder_update_status").val()
                },
                type: 'post',
                cache: false,
                success: function () {
                    window.location = "<?php echo admin_url("edit.php?post_type=sfba_subscribe_form") ?>";
                }
            })
        });
    });
